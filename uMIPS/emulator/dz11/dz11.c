...
struct {
	struct Line line[EMU_NUM_UARTS];
...
{
	struct Line *line = &gDZ11.line[EMU_NUM_UARTS - 1];
	uint8_t mask = 1 << (EMU_NUM_UARTS - 1);
	bool irq = false;
	int_fast8_t i;
	...
	if (gDZ11.enabled) {
		...
		for (i = EMU_NUM_UARTS - 1; i >= 0; i--, line--, mask >>= 1) {
...
static uint16_t dz11prvRx(void)	//assumes it will be followed by recalc
{
	struct Line *line = &gDZ11.line[EMU_NUM_UARTS - 1];
	int_fast8_t i;
	...
	for (i = EMU_NUM_UARTS - 1; i >= 0; i--, line--) {
		
		if (line->rxBytesUsed) {
			
			uint16_t ret = RBUF_DATAVALID;
			if (line->rxOvr) {
				line->rxOvr = false;
				ret |= RBUF_OVERFLOW;
			}
			ret |= (((uint16_t)line->buf[line->rxReadPtr]) << RBUF_RBUF_SHIFT) & RBUF_RBUF_MASK;
			if (++line->rxReadPtr == UART_RX_BUF_SZ)
				line->rxReadPtr = 0;
			line->rxBytesUsed--;
			line->nBytesRxedSinceLastRead = 0;
			ret |= (((uint16_t)i) << RBUF_RXLINE_SHIFT) & RBUF_RXLINE_MASK;
...
void dz11charRx(uint8_t lineNo, uint8_t chr)
{
	struct Line *line;
...
static bool dz11prvRealMemWrite(uint8_t regIdx, uint16_t v)
{
	uint8_t lineNo;
...
static bool dz11prvRealMemRead(uint8_t regIdx, uint16_t *valP)
{
	uint16_t v;
...
static bool dz11prvMemAccess(uint32_t pa, uint8_t size, bool write, void* buf)
{
	uint8_t regIdx;
	uint16_t val;
