...
#define EMU_NUM_IRQS				8		//lower 2 are sw irqs
...
void cpuIrq(uint8_t idx, bool raise)
{
	if (idx < EMU_NUM_IRQS) {
...
{
	uint8_t cause = wasWrite ? CP0_EXC_COD_TLBS : CP0_EXC_COD_TLBL;
...
static inline void cpuPrvTakeCoprocUnusableExc(uint8_t cpNo)
{
...
static inline uint8_t cpuGetRegNumT(uint32_t instr)
{
	return (instr >> 16) & (MIPS_NUM_REGS - 1);
}

static inline uint8_t cpuGetRegNumA(uint32_t instr)
{
	return (instr >> 6) & (MIPS_NUM_REGS - 1);
}

static inline uint8_t cpuGetRegNumD(uint32_t instr)
{
...
{
	uint8_t rn = cpuGetRegNumT(instr);
...
{
	uint8_t rn = cpuGetRegNumD(instr);
...
static void cpuPrvTlbHashRemove(uint8_t idx)
{
...
static void cpuPrvTlbHashAdd(uint8_t idx)
{
	uint8_t bucket = TLB_HASH(cpu.tlb[idx].va);
	int_fast8_t curPtr;
...
{
	uint8_t curAsid = (cpu.entryHi & TLB_ENTRYHI_ASID_MASK) >> TLB_ENTRYHI_ASID_SHIFT;
	int_fast8_t idx;
...
{
	uint8_t index = ((cpu.index >> 8) & 0x3f) % EMU_NUM_TLB_ENTRIES;
...
static uint8_t cpuPrvRefreshRandom(void)
{
	...
	rnd >>= 24;
	rnd %= (EMU_NUM_TLB_ENTRIES - EMU_NUM_WIRED_TLB_ENTRIES);
	rnd += EMU_NUM_WIRED_TLB_ENTRIES;
...
static bool cpuPrvDataAccess(void* buf, uint32_t va, uint8_t sz, bool write)
{
	uint32_t pa;
...
static bool cpuPrvCopAccess(uint8_t cpNo)
{
	if (cpNo) {
...
void cpuInit(uint32_t ramAmount)
{
	...
	cpu.tlb[EMU_NUM_TLB_ENTRIES - 1].nextIdx = -1;
